...
export async function getClub(slug:string){const {data,error}=await publicClient().from('clubs').select('id,slug,official_name,display_name,city,postal_code,department_code,region_code,claim_status,logo_path,logo_source').eq('slug',slug).eq('status','active').maybeSingle();if(error)throw error;return data}
