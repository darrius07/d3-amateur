...
if(mode==='setup'){
  ...
  const email=`step5a-e2e-${suffix}-owner@example.invalid`;
  const created=await service.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{display_name:'E2E Owner 5A'}});if(created.error)throw created.error;
  const ownerId=created.data.user.id;
  const club=await service.from('clubs').insert({slug:`d3-test-club-step5a-e2e-${suffix}`,official_name:`D3 Test Club Step5A E2E ${suffix}`,display_name:`D3 Test Club Step5A E2E ${suffix}`,status:'active',claim_status:'claimed'}).select('id,slug,display_name').single();if(club.error)throw club.error;
  ...
  const users=listed.data.users.filter(u=>u.email?.includes(`step5a-e2e-${suffix}-`));
  const clubs=await service.from('clubs').select('id').ilike('slug',`d3-test-club-step5a-e2e-${suffix}%`);
