...
if(mode==='setup'){
  ...
  const email=`step5b1-e2e-${suffix}-owner@example.invalid`;
  const created=await service.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{display_name:'E2E Owner 5B1'}});if(created.error)throw created.error;
  const ownerId=created.data.user.id;
  const club=await service.from('clubs').insert({slug:`d3-test-club-step5b1-e2e-${suffix}`,official_name:`D3 Test Club Step5B1 E2E ${suffix}`,display_name:`D3 Test Club Step5B1 E2E ${suffix}`,status:'active',claim_status:'claimed'}).select('id,slug,display_name').single();if(club.error)throw club.error;
  ...
  const players=[];
  for(let i=0;i<3;i++){
    const p=await service.rpc('manage_roster_player',{actor_id:ownerId,target_team_season_id:teamSeasonId,existing_player_id:null,new_first_name:'Fixture',new_last_name:`Player${i}${suffix}`,new_slug:`fixture-player${i}-${suffix}`,position_value:'MIDFIELDER',squad_value:null});
  ...
  const intruderEmail=`step5b1-e2e-${suffix}-intruder@example.invalid`;
  const intruderPassword=`D3!${crypto.randomUUID()}aA1`;
  const intruder=await service.auth.admin.createUser({email:intruderEmail,password:intruderPassword,email_confirm:true,user_metadata:{display_name:'E2E Intruder'}});if(intruder.error)throw intruder.error;
  console.log(JSON.stringify({suffix,email,password,ownerId,club:club.data,teamSeasonId,matchId:match.data,players,intruderEmail,intruderPassword}));
  ...
  const users=listed.data.users.filter(u=>u.email?.includes(`step5b1-e2e-${suffix}-owner`)||u.email?.includes(`step5b1-e2e-${suffix}-intruder`));
  const clubs=await service.from('clubs').select('id').ilike('slug',`d3-test-club-step5b1-e2e-${suffix}%`);
