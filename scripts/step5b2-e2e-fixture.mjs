...
if(mode==='setup'){
  ...
  const email=`step5b2-e2e-${suffix}-owner@example.invalid`;
  const created=await service.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{display_name:'E2E Owner 5B2'}});if(created.error)throw created.error;
  const ownerId=created.data.user.id;
  const club=await service.from('clubs').insert({slug:`d3-test-club-step5b2-e2e-${suffix}`,official_name:`D3 Test Club Step5B2 E2E ${suffix}`,display_name:`D3 Test Club Step5B2 E2E ${suffix}`,status:'active',claim_status:'claimed'}).select('id,slug,display_name').single();if(club.error)throw club.error;
  ...
  const players=[];
  for(let i=0;i<4;i++){
    const p=await service.rpc('manage_roster_player',{actor_id:ownerId,target_team_season_id:teamSeasonId,existing_player_id:null,new_first_name:'Fixture',new_last_name:`Player${i}${suffix}`,new_slug:`fixture-eventplayer${i}-${suffix}`,position_value:'MIDFIELDER',squad_value:null});
  ...
  const result=await service.rpc('enter_match_result',{actor_id:ownerId,p_match_id:matchId,p_home_score:2,p_away_score:1});if(result.error)throw result.error;
  ...
  const intruderEmail=`step5b2-e2e-${suffix}-intruder@example.invalid`;
  ...
  console.log(JSON.stringify({suffix,email,password,ownerId,club:club.data,teamSeasonId,matchId,players,intruderEmail,intruderPassword}));
  ...
  const users=listed.data.users.filter(u=>u.email?.includes(`step5b2-e2e-${suffix}-`));
  const clubs=await service.from('clubs').select('id').ilike('slug',`d3-test-club-step5b2-e2e-${suffix}%`);
